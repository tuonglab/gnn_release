...
def read_score_txt(path: str | Path) -> pd.DataFrame:
    ...
    return pd.read_csv(path, sep=",", header=None, names=["CDR3_Sequence", "Cancer_Score"])

def read_metrics_csv(directory: str | Path, filename: str = "metric_scores.csv") -> pd.DataFrame:
