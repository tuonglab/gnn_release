...
for i, graph in enumerate(graphs):
    ...
    if graph.x.numel() == 0:
        ...
        print(
            f"Label: {graph.y}, Characters: {getattr(graph, 'original_characters', 'N/A')}"
        )
        continue
    if graph.x.ndim != 2:
...
print(
    f"Filtered {len(graphs) - len(filtered_graphs)} graphs. Saving {len(filtered_graphs)} graphs back."
)
