...
def test(
    ...
    def create_filename(
        label: int, original_filename: str, iteration: int, suffix: str
    ) -> str:
        ...
        for root, _, files in os.walk(base_dir):
            ...
            if original_filename in csv_files:
                try:
                    ...
                    print(
                        f"Error reading CSV file {os.path.join(root, original_filename)}: {str(e)}"
                    )
                    continue
    ...
    for i, sample in enumerate(loader):
        ...
        if existing_scores:
            print(
                f"Skipping already processed file: {original_filename} (found: {existing_scores[0]})"
            )
            continue
        sample_scores = []
        sample_labels = []
        try:
            for j, data in enumerate(sample):
                ...
                if data.x.dim() != 2:
                    raise ValueError(
                        f"Input features must be 2-dimensional. Current shape: {data.x.shape}"
                    )
        ...
        if np.isnan(mean_sample_labels) or (
            isinstance(sample_labels, (list, np.ndarray))
            and np.all(np.isnan(sample_labels))
        ):
            print(f"Skipped file: {filenames[i]} (mean_sample_labels is NaN)")
            continue

        print(
            f"Processed file: {filenames[i]} | Mean Score: {mean_sample_scores:.4f} | Mean Label: {mean_sample_labels:.4f}"
        )
...
def test_trained_model(dataset_path, model_path, dataset_name) -> None:
    ...
    model = GATv2(
        nfeat=test_set[0][0].num_node_features, nhid=375, nclass=2, dropout=0.17
    )
    model.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset-path", type=str, required=True, help="Path to the test dataset."
    )
    parser.add_argument(
        "--model-path", type=str, required=True, help="Path to the saved model file."
    )
    parser.add_argument(
        "--dataset-name",
        type=str,
        required=True,
        help="Name of the dataset (used for naming output folders).",
    )
