...
gnn_no_clonal_weightage_pica = pd.read_csv("/scratch/project/tcr_ml/gnn_release/model_2025_sc_curated/PICA_scores/metric_scores.csv")
gnn_clonal_weightage_pica = pd.read_csv('/scratch/project/tcr_ml/gnn_release/icantcrscoring/model_2025_sc_curated/PICA/summary.csv')
icantcr_scoring_pica = pd.read_csv("/scratch/project/tcr_ml/iCanTCR/output/PICA_GLIPH/accumulated_pica_summary.csv")
...
plt.grid(axis="y", linestyle="--", alpha=0.7)
plt.tight_layout()
plt.savefig(f"model_scores_boxplot_{dataset_control.lower()}.pdf", format="pdf", dpi=600)
...
def plot_roc_between_groups(models, cancer_name, control_name):
    ...
    def combine_scores(cancer_scores, control_scores):
        ...
        labels = np.concatenate([np.ones(len(cancer_scores)), np.zeros(len(control_scores))])
